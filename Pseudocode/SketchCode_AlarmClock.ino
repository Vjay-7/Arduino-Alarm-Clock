...
byte edit(byte x, byte y, byte parameter){
  ...
  while(true){
    while(!digitalRead(button2)){                           // If button B2 is pressed
      parameter++;
      if(((i == 3) || (i == 5)) && parameter > 23)          // If hours > 23 ==> hours = 0
        parameter = 0;
      if(((i == 4) || (i == 6)) && parameter > 59)          // If minutes > 59 ==> minutes = 0
        parameter = 0;
...
void loop() {
  ...
  if(!digitalRead(button1)){ 
      ...
      month = edit(5, 0, month);                // Edit month
      date = edit(8, 0, date);                  // Edit date
      year = edit(13, 0, year);                 // Edit year
      hour   = edit(4, 1, hour);
      minute = edit(7, 1, minute);
    ...
    while(!digitalRead(button3));             // Wait until button B3 released              
     alarms_read_display();                        // Read and display alarms parameters
      alarm1_hour   = edit(4,  1, alarm1_hour);
      alarm1_minute = edit(7,  1, alarm1_minute);
      alarm1_status = edit(13, 1, alarm1_status);
      ...
      Wire.write(4 | alarm1_status);      // Write data to DS3231 control register (enable interrupt when alarm)
    ...
    if(!digitalRead(button2) && digitalRead(alarm_pin)){         // When button B2 pressed with alarm (Reset and turn OFF the alarm)
      ...
      Wire.write(4 | (!bitRead(status_reg, 0) & alarm1_status));
      Wire.write(0);                              // Clear alarm flag bits
      Wire.endTransmission();                     // Stop transmission and release the I2C bus
    }
